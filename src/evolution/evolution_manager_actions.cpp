...
void EvolutionManager::compute_actions_for_population(PopulationEvolutionState &population, AgentRegistry &agents,
  ...
  population.network_cache.activate_batch(entity_count, sensors, all_outputs, SENSOR_COUNT, OUTPUT_COUNT);
...
void EvolutionManager::compute_actions(AppState &state, const std::vector<float> &predator_sensors,
  ...
  compute_actions_for_population(state.evolution.predators, state.predator, predator_sensors, predator_decisions);
  compute_actions_for_population(state.evolution.prey, state.prey, prey_sensors, prey_decisions);
}
