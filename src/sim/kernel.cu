...
uint32_t allocate_population_buffers(DevicePopulationBuffers &population, std::uint32_t capacity, std::uint32_t num_inputs, std::uint32_t node_stride, std::uint32_t connection_stride, std::uint32_t output_stride) {
  ...
  for (auto status : {
           ...
           moonai_gpu::alloc_array(&population.genome.connection_innovation, static_cast<std::size_t>(capacity) * connection_stride),
           moonai_gpu::alloc_array(&population.genome.connection_enabled, static_cast<std::size_t>(capacity) * connection_stride),
           ...
           moonai_gpu::alloc_array(&population.compiled.connection_offsets, static_cast<std::size_t>(capacity) * (node_stride + 1U)),
           moonai_gpu::alloc_array(&population.compiled.output_indices, static_cast<std::size_t>(capacity) * output_stride),
           moonai_gpu::alloc_array(&population.compiled.connection_sources, static_cast<std::size_t>(capacity) * connection_stride),
           moonai_gpu::alloc_array(&population.compiled.connection_weights, static_cast<std::size_t>(capacity) * connection_stride),
...
uint32_t allocate_food_buffers(FoodBuffer &food, std::uint32_t capacity) {
...
void free_reproduction_buffers(DeviceState &state) {
...
uint32_t ensure_reproduction_buffers(DeviceState &state) {
...
uint32_t ensure_metrics_buffer(DeviceState &state) {
...
uint32_t ensure_spatial_grid_buffers(DeviceState &state, std::uint32_t cell_count) {
  if (cell_count == 0U) {
...
uint32_t copy_population_buffers(const DevicePopulationBuffers &src, DevicePopulationBuffers &dst, std::uint32_t num_inputs) {
...
uint32_t zero_population_tail(const DevicePopulationBuffers &population, std::uint32_t from_capacity,
...
uint32_t replace_population_buffers(DevicePopulationBuffers &population, DevicePopulationBuffers replacement,
                                      std::uint32_t num_inputs) {
...
extern "C" std::int32_t dev_ensure_counter_buffer(DeviceState *state) {
...
extern "C" std::int32_t dev_ensure_free_lists(DeviceState *state) {
...
extern "C" std::int32_t dev_ensure_reproduction_buffers(DeviceState *state) {
  return ensure_reproduction_buffers(*state);
}

extern "C" std::int32_t dev_ensure_metrics_buffer(DeviceState *state) {
  return ensure_metrics_buffer(*state);
}

extern "C" std::int32_t dev_ensure_spatial_grid_buffers(DeviceState *state) {
...
extern "C" std::int32_t dev_expand_population(DeviceState *state, PopulationKind population_kind, std::uint32_t new_capacity) {
  auto &population = moonai_gpu::population_for_kind(*state, population_kind);
